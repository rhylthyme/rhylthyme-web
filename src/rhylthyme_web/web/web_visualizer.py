#!/usr/bin/env python3
"""
DAG (Directed Acyclic Graph) visualizer for Rhylthyme programs.

This module generates static D3.js visualizations showing the task dependency
structure of Rhylthyme programs as directed acyclic graphs.
"""
...
# Try local modules first (for Vercel deployment), fallback to rhylthyme package
try:
    from rhylthyme_web.rhylthyme import load_program_file, get_environment_icon
except ImportError:
    from rhylthyme.program_runner import load_program_file
    from rhylthyme.environment_icons import get_environment_icon

def extract_step_dependencies(program: Dict[str, Any]) -> Tuple[List[Dict], List[Dict]]:
    """
    Extract steps and their dependencies from a program.
    
    Args:
        program: The program dictionary
        
    Returns:
        Tuple of (nodes, edges) where:
        - nodes: List of step nodes with id, name, track info
        - edges: List of dependency edges with source and target step IDs
    """
    nodes = []
    edges = []
    step_to_track = {}
    ...
    # Extract all steps and track their relationships
    for track in program.get('tracks', []):
        ...
        for step in track.get('steps', []):
            ...
            task = step.get('task', step.get('tasks', ['']))
            ...
            if duration:
                ...
                if duration_type == 'fixed':
                    ...
                    min_sec = duration.get('minSeconds', 0)
                    max_sec = duration.get('maxSeconds', 0)
                    default_sec = duration.get('defaultSeconds', 0)
                    duration_info = f"{min_sec}-{max_sec}s (default: {default_sec}s)"
            
            # Extract buffer information
            pre_buffer = step.get('preBuffer', {})
            post_buffer = step.get('postBuffer', {})
            
            step_to_track[step_id] = track_id
            ...
            nodes.append({
                ...
                'flex': step.get('flex', {'enabled': False})
            })
            
            # Analyze start trigger to create edges
            start_trigger = step.get('startTrigger', {})
            edges.extend(extract_dependencies_from_trigger(start_trigger, step_id))
    
    return nodes, edges

def extract_dependencies_from_trigger(trigger: Dict[str, Any], target_step: str) -> List[Dict]:
    """
    Extract dependency edges from a start trigger.
    
    Args:
        trigger: The start trigger configuration
        target_step: The step ID that this trigger applies to
    ...
    if trigger_type == 'programStart':
        ...
        source_step = trigger.get('stepId')
        if source_step:
            edges.append({
                'source': source_step,
                'target': target_step,
    ...
    elif trigger_type == 'afterStepWithBuffer':
        source_step = trigger.get('stepId')
        ...
        # Manual triggers don't have dependencies from other steps
        pass
    elif trigger.get('logic'):  # Multiple triggers with logic
        logic_type = trigger.get('logic')
        triggers = trigger.get('triggers', [])
        for sub_trigger in triggers:
            edges.extend(extract_dependencies_from_trigger(sub_trigger, target_step))
    
    return edges

def parse_duration_string(duration_str) -> int:
    """Parse duration string like '30s', '2m', '1h' or integer into seconds."""
    if not duration_str:
        return 0
    ...
    # Handle complex format like "90s)" by removing trailing characters
    ...
    if duration_str.endswith('s'):
        ...
        # Try to extract just the number part if no unit
        import re
...
def calculate_timeline_data(nodes: List[Dict], edges: List[Dict]) -> Dict:
    """
    Calculate timeline data for steps based on dependencies and durations.
    ...
    Returns:
        Dictionary with timeline data including tracks, steps with calculated start times
    """
    ...
    for edge in edges:
        target = edge['target']
        if target not in step_dependencies:
            step_dependencies[target] = []
        
        if edge['type'] == 'programStartOffset':
            step_offsets[target] = edge.get('offset', 0)
        elif edge['type'] == 'afterStepWithBuffer':
            step_buffers[(edge['source'], target)] = edge.get('buffer', 0)
        
        if edge['source'] != 'program_start':
            step_dependencies[target].append(edge['source'])
    
    # Calculate start times for each step
    step_start_times = {'program_start': 0}
    step_durations = {}
    step_buffers = {}
    
    # Extract durations and buffer information from nodes
    for node in nodes:
        if node['type'] == 'step':
            duration_str = node.get('duration', '0s')
            if duration_str:
                # Parse duration - handle both "60s" and "120-180s (default: 150s)" formats
                if 'default:' in duration_str:
                    # Variable duration, extract default
                    default_part = duration_str.split('default:')[1].strip()
                    duration = parse_duration_string(default_part)
                else:
                    # Fixed duration
                    duration = parse_duration_string(duration_str)
                step_durations[node['id']] = duration
            
            # Extract buffer durations
            pre_buffer = node.get('preBuffer', {})
            post_buffer = node.get('postBuffer', {})
            
            step_buffers[node['id']] = {
                'preBuffer': pre_buffer,
                'postBuffer': post_buffer
            }
    ...
    def calculate_start_time(step_id):
        if step_id in temp_visited:
            return 0  # Cycle detected, return 0
        if step_id in visited:
            return step_start_times.get(step_id, 0)
        
        temp_visited.add(step_id)
        
        max_predecessor_end = 0
        
        # Handle program start offset
        if step_id in step_offsets:
    ...
    for node in nodes:
        if node['type'] == 'step':
            ...
            if track_id not in tracks:
                tracks[track_id] = {
                    'trackId': track_id,
                    'name': node['track_name'],
                    'steps': []
                }
            
            start_time = step_start_times.get(node['id'], 0)
            duration = step_durations.get(node['id'], 0)
            
            # Get buffer information
            buffers = step_buffers.get(node['id'], {})
            pre_buffer = buffers.get('preBuffer', {})
            post_buffer = buffers.get('postBuffer', {})
            
            # Parse buffer durations
            pre_buffer_duration = parse_duration_string(pre_buffer.get('duration', ''))
            post_buffer_duration = parse_duration_string(post_buffer.get('duration', ''))
            
            # Calculate buffer start times
            pre_buffer_start = start_time - pre_buffer_duration if pre_buffer_duration > 0 else None
            post_buffer_start = start_time + duration if post_buffer_duration > 0 else None
            ...
            step_data = {
                ...
                'task': node.get('task', ''),
                'flex': node.get('flex', {'enabled': False})
            }
            
            # Add buffer information if present
            if pre_buffer_duration > 0:
                step_data['preBuffer'] = {
                    'startTime': pre_buffer_start,
                    'duration': pre_buffer_duration,
                    'endTime': start_time,
                    'description': pre_buffer.get('description', ''),
                    'tasks': pre_buffer.get('tasks', [])
                }
            
            if post_buffer_duration > 0:
                step_data['postBuffer'] = {
                    'startTime': post_buffer_start,
                    'duration': post_buffer_duration,
                    'endTime': post_buffer_start + post_buffer_duration,
                    'description': post_buffer.get('description', ''),
                    'tasks': post_buffer.get('tasks', [])
                }
            
            tracks[track_id]['steps'].append(step_data)
    
    # Calculate total timeline duration including all buffers
    max_end_time = 0
    min_start_time = 0
    
    for track_data in tracks.values():
        for step in track_data['steps']:
            # Check for pre-buffer extending timeline start
            if 'preBuffer' in step:
                min_start_time = min(min_start_time, step['preBuffer']['startTime'])
            
            # Check main step and post-buffer extending timeline end
            max_end_time = max(max_end_time, step['endTime'])
            if 'postBuffer' in step:
                max_end_time = max(max_end_time, step['postBuffer']['endTime'])
    
    # Adjust all times to start from 0 if there are negative pre-buffer times
    time_offset = -min_start_time if min_start_time < 0 else 0
    
    if time_offset > 0:
        for track_data in tracks.values():
            for step in track_data['steps']:
                step['startTime'] += time_offset
                step['endTime'] += time_offset
                if 'preBuffer' in step:
                    step['preBuffer']['startTime'] += time_offset
                    step['preBuffer']['endTime'] += time_offset
                if 'postBuffer' in step:
                    step['postBuffer']['startTime'] += time_offset
...
def generate_dag_html(nodes: List[Dict], edges: List[Dict], program_data: Dict[str, Any], environment_data: Dict[str, Any] = None, resource_constraints: List[Dict[str, Any]] = None) -> str:
    """
    Generate HTML with D3.js DAG visualization.
    
    Args:
        nodes: List of step nodes
        edges: List of dependency edges
        program_data: The full program data dictionary
        environment_data: The environment data dictionary (optional)
        resource_constraints: List of resource constraints (optional)
    ...
    program_version = program_data.get('version', 'N/A')
    environment_type = program_data.get('environmentType', 'N/A')
    default_environment = program_data.get('defaultEnvironment', 'N/A')
    
    # Get environment icon
    environment_icon = get_environment_icon(environment_type) if environment_type != 'N/A' else 'fa-building'
    
    # Add environment information if available
    environment_name = 'N/A'
    environment_description = ''
    if environment_data:
        environment_name = environment_data.get('name', environment_data.get('environmentId', 'N/A'))
        environment_description = environment_data.get('description', '')
    elif program_data.get('environment'):
        environment_name = program_data.get('environment')
    ...
    timeline_data = calculate_timeline_data(nodes, edges)
    timeline_json = json.dumps(timeline_data, indent=2)
    
    # Prepare environment and resource data for JavaScript
    environment_json = json.dumps(environment_data or {}, indent=2)
    
    # Transform resource constraints to expected format for JavaScript
    transformed_constraints = []
    for constraint in (resource_constraints or []):
        # Handle both new format (name/capacity) and old format (task/maxConcurrent)
        if 'name' in constraint and 'capacity' in constraint:
            # New format - transform to JavaScript expected format
            transformed_constraints.append({
                'task': constraint['name'],
                'maxConcurrent': constraint['capacity'],
                'description': constraint.get('description', '')
            })
        elif 'task' in constraint and 'maxConcurrent' in constraint:
            # Old format - use as is
            transformed_constraints.append(constraint)
    
    resource_constraints_json = json.dumps(transformed_constraints, indent=2)
    
    # Calculate dynamic left margin based on longest track name
    max_track_name_length = 0
    if timeline_data.get('tracks'):
        for track in timeline_data['tracks']:
            track_name = track.get('name', '')
            max_track_name_length = max(max_track_name_length, len(track_name))
    
    # Estimate pixels needed for track names (approximately 8px per character + padding)
    estimated_track_label_width = max(120, max_track_name_length * 8 + 40)
    dynamic_left_margin = min(estimated_track_label_width, 300)  # Cap at 300px to avoid extreme widths
    
    # Format total duration for display
    total_duration = timeline_data.get('totalDuration', 0)
    ...
    def format_duration(seconds):
        if seconds >= 3600:
            ...
            minutes = seconds / 60
            return f"{minutes:.1f} minutes" if minutes != int(minutes) else f"{int(minutes)} minutes"
        else:
            return f"{int(seconds)} seconds"
    
    formatted_duration = format_duration(total_duration)
    
    # Check for program fixed duration
    program_duration_text = ""
    if program_data.get('duration'):
        duration_config = program_data['duration']
        if duration_config.get('type') == 'fixed':
            if duration_config.get('seconds'):
                program_duration_seconds = duration_config['seconds']
                program_duration_text = format_duration(program_duration_seconds)
            elif duration_config.get('timeString'):
                program_duration_text = duration_config['timeString']
    
    # Generate relevant preset buttons based on total duration
    preset_buttons = []
    duration_minutes = total_duration / 60
    
    # Define all possible presets
    all_presets = [2, 5, 15, 30, 60, 120, 240, 480]

    preset_buttons_html = '\n                    '.join(preset_buttons)
    
    # Build the HTML template using string formatting to avoid f-string conflicts with JavaScript
...
<body class="bg-gray-100 font-sans">
    ...
    html_template += f"""
        <h1 class="text-3xl font-bold text-gray-800 mb-2 pb-2 border-b-2 flex items-center gap-3" style="border-color: #4B46B9;">
            <i class="fas {environment_icon} text-gray-600" title="Environment Type: {environment_type}"></i>
            {program_name}
        </h1>
        ...
        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 text-sm">
            <div class="flex flex-col items-center text-center">
                <span class="font-bold text-gray-600 uppercase text-xs tracking-wide mb-1">{"Program Duration" if program_duration_text else "Estimated Total Time"}</span>
                <div class="flex flex-col items-center">
                    {f'<span class="text-blue-600 font-semibold text-sm mb-1">{program_duration_text}</span>' if program_duration_text else ''}
                    <span class="text-dag-green font-semibold text-sm">{f"Est: {formatted_duration}" if program_duration_text else formatted_duration}</span>
                </div>
            </div>
            <div class="flex flex-col items-center text-center">
                <span class="font-bold text-gray-600 uppercase text-xs tracking-wide mb-1">Version</span>
                <span class="font-semibold text-sm" style="color: #4B46B9;">{program_version}</span>
            </div>
            <div class="flex flex-col items-center text-center">
                <span class="font-bold text-gray-600 uppercase text-xs tracking-wide mb-1">Environment Type</span>
                <span class="text-dag-purple font-semibold text-sm">{environment_type}</span>
            </div>
            <div class="flex flex-col items-center text-center">
                <span class="font-bold text-gray-600 uppercase text-xs tracking-wide mb-1">Environment</span>
                <span class="text-dag-purple font-semibold text-sm" title="{environment_description}">{environment_name}</span>
            </div>
            <div class="flex flex-col items-center text-center">
        ...
        <div class="flex flex-col lg:flex-row gap-6 mb-6 relative" id="panels-container">
            ...
            <div class="flex-1 min-w-0" id="timeline-panel">
                ...
                <div class="w-full h-96 border border-gray-300 rounded overflow-x-auto overflow-y-hidden relative">
                    ...
                    <div class="absolute top-2 right-2 bg-black bg-opacity-70 text-white py-1 px-2 rounded text-xs pointer-events-none opacity-0 transition-opacity duration-300 hover:opacity-100" id="scroll-hint">← Scroll horizontally →</div>
                </div>
            </div>
    ...
    <script>
        ...
        const margin = { top: 50, right: 50, bottom: 30, left: """ + str(dynamic_left_margin) + """ };
...
def generate_dag_visualization(program_file: str, output_file: str = None, open_browser: bool = True) -> str:
    """
    Generate a DAG visualization from a program file.
    
    Args:
        program_file: Path to the program JSON file
    ...
    program_name = program.get('name', os.path.basename(program_file))
    program_description = program.get('description', '')
    
    # Load environment and resource constraints if available
    ...
    try:
        ...
        # Load full environment data if environment is specified
        ...
        if environment_id:
            ...
            try:
                ...
                print(f"Warning: Could not load environment '{environment_id}'")
                
    except ImportError:
        print("Warning: Environment loader not available, using embedded constraints only")
        resource_constraints = program.get('resourceConstraints', [])
    
    # Extract dependencies
    nodes, edges = extract_step_dependencies(program)
    
    # Generate HTML with environment data
    html_content = generate_dag_html(nodes, edges, program, environment_data, resource_constraints)
    
    # Determine output file
    if output_file is None:
        program_path = Path(program_file)
        output_file = program_path.parent / f"{program_path.stem}_dag.html"
    
    # Write HTML file
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print(f"DAG visualization generated: {output_file}")
    print(f"Found {len(nodes)} steps and {len(edges)} dependencies")
    if resource_constraints:
        print(f"Found {len(resource_constraints)} resource constraints")
    if environment_data:
        print(f"Using environment: {environment_data.get('name', environment_id)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate DAG visualization from Rhylthyme program files")
    ...
    try:
        ...
        print(f"Success! DAG visualization saved to: {output_file}")
